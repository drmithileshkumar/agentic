...
Groq \texttt{gpt-oss-20b}  & 9/50 = 18.0\% & 9/28 = 32.1\% & 28 \\
\hline
\end{tabular}
\end{table}

The gap between the two columns is the whole point.
Groq leads on both metrics (18.0\% vs.\ 14.0\% headline; 32.1\% vs.\ 31.8\%
reachable), but the 4-percentage-point headline gap narrows to 0.3~pp once the
infrastructure-dominated denominator is removed --- a difference not significant
at $\alpha = 0.05$. The headline gap is thus almost entirely an artefact of
differential infrastructure exposure between runs, not a capability difference.

Groq's figure incorporates one correction.\footnote{Groq \texttt{proof\_0006}
step~3 applied \texttt{exact fun x hx => h2 (h1 hx)}, closing
\texttt{SET\_E\_03} with no goals remaining; the harness recorded
\texttt{advanced} and the attempt continued to budget exhaustion. The tactic is
axiom-free and independently confirmed by \texttt{lake env lean}. The cause is
\emph{not} the pre-\texttt{dd33000} misclassification discussed in
\S\ref{sec:results_rq4} --- this run postdates that fix by twenty hours. Step
outcomes are decided by \texttt{proof\_status} being \texttt{Completed} alone,
so a REPL
response carrying an empty goal list without that status falls through to
\texttt{advanced} with a null goal. The defect is still present.}
...
\texttt{SET\_E\_03} & Set theory       & both \\
...
Seven of the nine were proved by both models, and the two that were not ---
\texttt{TOP\_E\_02} and \texttt{ABA\_E\_01} --- were both proved by Groq alone.
Nemotron proved nothing the smaller model did not also prove, and neither
singleton is obviously harder than the shared seven.
...
The exclusion is made on the proof's terminal result code, not by inspecting
message text: an attempt ending in \texttt{FAILED\_LLM\_ERROR},
\texttt{FAILED\_CONNECTIVITY\_OUTAGE} or \texttt{FAILED\_ELABORATION} is dropped
before \texttt{classify\_failures.py} runs, leaving only those ending in
\texttt{FAILED\_BUDGET\_EXHAUSTED}.
...
\textsc{Error Loop} cases --- but during the NVIDIA NIM outage every affected
step recorded the identical string \texttt{LLM call failed: NotFoundError: Error
code: 404}, and three such steps in a row satisfy the \textsc{Error Loop} test
exactly. All 22 of the cases the filter removes are HTTP 404s of this kind, so
most of those 28 are outages wearing the costume of a reasoning failure.
Restricting to the 35 failures the model is actually responsible for gives:
...
\hline
\textbf{Advanced rate} & 6.5\% & 7.1\% & 2.4\% & 11.1\% & 0.0\% & 0.0\% \\
\hline
\end{tabular}
\end{center}

\noindent (Nemotron run; the denominator shrinks as proofs terminate.
The Nemotron run predates harness fix \texttt{dd33000} (2026-09-12), which
corrected a rule that logged failed tactics as \texttt{advanced};
\texttt{reclassify\_run.py} corrects 13 of 316 steps, and the figures above are
the corrected ones. Uncorrected, step~1 reads 23.9\%.)
The rate never exceeds 11\% at any step, and no step after the fourth produced a
single advance. The agent does not improve as the episode goes on; it makes a
few early gains and then stops making any.
